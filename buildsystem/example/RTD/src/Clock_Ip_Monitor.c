...
#ifdef CLOCK_IP_CGU_INTERRUPT
static void Clock_Ip_ResetCguInterrupt(Clock_Ip_CmuConfigType const* Config);
static void Clock_Ip_EnableCguInterrupt(Clock_Ip_CmuConfigType const* Config);
#endif
...
static void Clock_Ip_ResetCguInterrupt(Clock_Ip_CmuConfigType const* Config)
{
    (void)Config;
    /* Disable interupts */
    IP_CGU->INT_CLR_ENABLE = CLOCK_IP_CGU_INT_CLR_ENABLE_RWBITS_MASK32;
    /* Clear status */
    IP_CGU->INT_CLR_STATUS = CLOCK_IP_CGU_INT_CLR_STATUS_RWBITS_MASK32;
}

static void Clock_Ip_EnableCguInterrupt(Clock_Ip_CmuConfigType const* Config)
{
    uint32 CguEnableValue;
    
    /* Get all interrupt is enabled */
    CguEnableValue = IP_CGU->INT_ENABLE;
    /* Disable interupts, Clear status*/
    Clock_Ip_ResetCguInterrupt(Config);
    /* Enable interrupts */
    IP_CGU->INT_SET_ENABLE = (CguEnableValue | (Config->Interrupt)) ;
}
#endif
...
void Clock_Ip_CMU_ClockFailInt(void)
{
    ...
    {
        if((Clock_Ip_apxCmu[IndexCmu] == (Clock_Ip_ClockMonitorType*)IP_CMU_FC_27) || (Clock_Ip_apxCmu[IndexCmu] == (Clock_Ip_ClockMonitorType*)IP_CMU_FC_28))
        {
            /* Check Partition 1 Status is enabled */
            if ((IP_MC_ME->PRTN1_STAT) & MC_ME_PRTN1_STAT_PCS_MASK)
            {  
                /* Read flags */
                CmuIsrValue = Clock_Ip_apxCmu[IndexCmu]->SR & CLOCK_IP_CMU_ISR_MASK;
...
#ifdef CLOCK_IP_CGU_INTERRUPT
    {
        Clock_Ip_ResetCguInterrupt,                /* Reset */
        Clock_Ip_ClockMonitorEmpty,                /* Set */
        Clock_Ip_ClockMonitorEmpty_Disable,        /* Disable */
        Clock_Ip_ClockMonitorEmpty_ClearStatus,    /* Clear */
        Clock_Ip_ClockMonitorEmpty_GetStatus,      /* Get status */
        Clock_Ip_EnableCguInterrupt,               /* Enable */
    },
#endif
